...
public class SuccessCurve implements CurveMeasure {
	...
	public SuccessCurve(String name) {
		this.name = name;
	...
	public MeasureSet getValue(String run, String[] xValues)
		...
		String command = "./trec_eval -m success." + xValues[0];
		...
		topicSet.setMeasureName("sucess");
		return topicSet;
	}
 ...
 InterruptedException,
		...
		Run r = this.context.getRunByName(run);
		String command = "./trec_eval -q -m success." + xValues[0];
		for (int i = 1; i < xValues.length; i++) {
			if(xValues[i] != null && !xValues[i].equals("null"))
				command += "," + xValues[i];
		}
